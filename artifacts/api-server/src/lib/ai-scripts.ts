...
- Engagement medio: ${strategyContext.account_data.avg_engagement.toFixed(1)}%; alcance medio: ${Math.round(strategyContext.account_data.avg_reach)}.
- Captions propios destacados: ${strategyContext.account_data.top_captions.slice(0, 3).map((caption) => caption.substring(0, 160)).join(" | ") || "sin datos suficientes"}.
- Señales de posts fuertes/débiles: ${strategyContext.account_data.top_posts.slice(0, 2).map((post) => `"${post.caption?.substring(0, 90) ?? "sin caption"}" (${post.engagement_rate ?? "sin métrica"}%)`).join(" | ") || "sin datos suficientes"}.
- Ángulos débiles a no repetir: ${strategyContext.account_data.top_posts.slice(-2).map((post) => `"${post.caption?.substring(0, 90) ?? "sin caption"}" (${post.engagement_rate ?? "sin métrica"}%)`).join(" | ") || "sin datos suficientes"}.
Usa estas métricas como evidencia correlacional, no como garantía causal; si faltan datos, no inventes resultados.
...
EVIDENCIA PROPIA DISPONIBLE:
- Engagement medio: ${options.strategyContext.account_data.avg_engagement.toFixed(1)}%; alcance medio: ${Math.round(options.strategyContext.account_data.avg_reach)}.
- Captions propios destacados: ${options.strategyContext.account_data.top_captions.slice(0, 3).map((caption) => caption.substring(0, 160)).join(" | ") || "sin datos suficientes"}.
- Posts fuertes/débiles disponibles: ${options.strategyContext.account_data.top_posts.slice(0, 3).map((post) => `"${post.caption?.substring(0, 90) ?? "sin caption"}" (${post.engagement_rate ?? "sin métrica"}%)`).join(" | ") || "sin datos suficientes"}.
- No repitas literalmente los posts ganadores ni priorices los ángulos de los posts más débiles: varía el enfoque y usa sus métricas como señal, no como causalidad.
Usa métricas y patrones como evidencia correlacional, no copies contenido ni prometas resultados.
...
Devuelve SOLO el mismo JSON válido con hook, script, cta, caption, hashtags y estimated_duration_seconds.`;
  ...
  logger.info({
    ...
    hasHookCandidates: hookCandidatesList.length > 0,
    strategicContextVersion: options?.strategyContext?.context_version ?? "semantic-only",
    sourcesUsed: [
      "creator_profile",
      ...(options?.strategyContext ? ["instagram_account_audit", "market_study", "content_strategy", "radar_competitors"] : []),
      ...(options?.auditInsights ? ["audit_cache"] : []),
      ...(options?.topicKeywords?.length ? ["topic_keywords"] : []),
    ],
  }, "Script generated by AI with strategic trace");
...
export async function generateContentTopics(
  ...
  // Keywords are signals for vocabulary and subtopics, never equal-priority pillars.
  const allPillars = rawPillars.length ? rawPillars : ["contenido alineado con la oferta y la audiencia"];
...
- Formatos que funcionan en este nicho: ${strategyContext.market_insights.working_formats.slice(0, 4).join("; ")}
 - Temas con mejor señal en el estudio: ${strategyContext.market_insights.top_themes.slice(0, 4).join("; ")}
 - Contexto versionado: ${strategyContext.context_version}
 - Evidencia propia: engagement ${strategyContext.account_data.avg_engagement.toFixed(1)}%, alcance ${Math.round(strategyContext.account_data.avg_reach)}, ${strategyContext.account_data.analyzed_posts} publicaciones analizadas.
 Usa patrones como evidencia correlacional; no copies literalmente a referentes ni asumas causalidad.
`
    : "";
...
Genera exactamente ${total} temas distintos. Verifica la lista completa antes de responder para confirmar que no hay duplicados ni temas similares.`;
  ...
  logger.info({
    strategicContextVersion: strategyContext?.context_version ?? "semantic-only",
    sourcesUsed: [
      "creator_profile",
      "content_history",
      ...(strategyContext ? ["instagram_account_audit", "market_study", "content_strategy", "radar_competitors"] : []),
      ...(auditInsights ? ["audit_cache"] : []),
    ],
    topicsRequested: total,
  }, "Content topics generated with strategic trace");
