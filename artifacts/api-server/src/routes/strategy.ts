  ...
  try {
    const marketInsights = await synthesizeMarketStudy({
      ...
      topicKeywords:     (settingsRow?.topicKeywords as string[] | null) ?? [],
       offer:             settingsRow?.offer ?? null,
       idealAudience:     settingsRow?.idealAudience ?? null,
       uniqueValueProp:   settingsRow?.uniqueValueProp ?? null,
       voiceStyle:        settingsRow?.voiceStyle ?? null,
       commonObjections:  settingsRow?.commonObjections ?? null,
